...
*   **js/core/Game.js**: 게임 상태, UI 오케스트레이션, 시즌 흐름, 저장 훅.
    한 클래스에 화면·로스터·시즌이 다 들어 있어 6,500줄이 넘습니다. 쪼개는 것은
    아직 하지 않았고(테스트가 없어 한 번에 옮기기 위험합니다), 대신 **여러 곳이
    같은 것을 각자 적던 자리부터 걷어내는 중**입니다 — 아래 두 개가 그 결과입니다.
*   **js/core/playerCard.js**: 선수 목록 카드의 공용 조각(스탯 줄 · 부상/피로 배지 ·
    빈 목록 줄). 목록 렌더러들이 같은 HTML 을 각자 적고 있어서, 한 곳만 고치면
    목록마다 카드 모양이 갈렸습니다. DOM 을 만들지 않고 문자열만 돌려줍니다.
*   **`Game.createNeedScorer()`**: 로스터의 빈자리를 메우는 쪽으로 기운 선수 평가
    함수를 만듭니다. AAA 콜업과 드래프트 자동 지명이 같은 30줄을 각자 들고 있었는데,
    한쪽만 손보면 "콜업은 포수가 모자란 걸 아는데 드래프트는 모르는" 식으로 갈립니다.
...
*   **server/**: 세이브 동기화 백엔드(`node:http` · 세션 쿠키 로컬 검증. 의존성은 `pg`·`dotenv` 뿐).
...
통합 로그인 토큰의 서명·만료·payload 검증은 `cd server && npm test`로 확인합니다.
